...
from EEG_Encoder_Transformer import EEGFeatureExtractor
...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.align = AlignmentHead(in_dim=256, hidden_dim=256, embed_dim=256, n_emotions=4)
    self.eegencoder = EEGFeatureExtractor()
    ckpt = torch.load("/public/home/xlzhu/sgevc/Sgevc/logs/Stage2_Transformer/stage2_epoch_600.pt", map_location="cpu")
    self.eegencoder.load_state_dict(ckpt["eeg_encoder"], strict=False)
  ...
  def forward(self, x, x_lengths, y, y_lengths, sid, temp, eeg):
    """
        x:[64,97] 
        y:[64,513,400]
    """
    ...
    eu, logits_speech = self.align(eu)
    # Adjust eeg dimensions if necessary
    eeg = eeg.squeeze(1) if eeg.dim() == 4 else eeg
            

    # Adjust eeg dimensions to match expected shape
    eeg = eeg.squeeze(-1)  # Convert [B, 30, 2500, 1] to [B, 2500, 30]
    eeu, sublogit = self.eegencoder(eeg)
    g = gs+eu
    g_eeg = gs+eeu
    ...
    w = attn.sum(2) #[b,1,t]
    if self.use_sdp:
      l_length = self.dp(x, x_mask, w, g=g_eeg)
      l_length = l_length / torch.sum(x_mask)
    else:
      logw_ = torch.log(w + 1e-6) * x_mask
      logw = self.dp(x, x_mask, g=g_eeg)
    ...
    o = self.dec(z_slice, g=g_eeg)
    return o, l_length, attn, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q, e, log_q_e, q_e), sublogit

  def infer(self, x, x_lengths, y, y_lengths, sid, eeg, y2=None, y2_lengths=None, temp=1.0, noise_scale=0.67, length_scale=1, noise_scale_w=0.8, max_len=None):
    """
    """
    label=None
    gs = self.emb_gs(sid).unsqueeze(-1) # [b, h, 1]
    ...
    g_eeg=gs+eeu
    if self.use_sdp:
      logw = self.dp(x, x_mask, g=g_eeg, reverse=True, noise_scale=noise_scale_w)
    else:
      logw = self.dp(x, x_mask, g=g_eeg)
    ...
    z = self.flow(z_p, y_mask, g=g_eeg, reverse=True)
    
    o = self.dec((z * y_mask)[:,:,:max_len], g=g_eeg)
    return o, attn, y_mask, (z, z_p, m_p, logs_p), eu, e, log_q_e, q_e, sublogit

  def voice_conversion(self, y, y_lengths, y1, y1_lengths,eeg, sid_src, sid_trg):
