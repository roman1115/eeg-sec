...
from model_stage3_Linear import (
  SynthesizerTrn,
...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers, ser):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid, eid, eeg) in enumerate(train_loader):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q, e, log_q_e, q_e) = net_g(x, x_lengths, spec, spec_lengths, sid, temp, eeg)
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      with autocast(enabled=False):
        ...
        loss_gen_all = loss_gen + loss_fm + loss_mel + loss_dur + loss_kl  + loss_sup + loss_unsup + loss_emotion
    optim_g.zero_grad()
    ...
    if rank==0:
      if global_step % hps.train.log_interval == 0:
        ...
        losses = [loss_disc, loss_gen, loss_fm, loss_mel, loss_dur, loss_kl, loss_sup, loss_unsup, loss_emotion]
        logger.info('Train Epoch: {} [{:.0f}%]'.format(
          epoch,
          100. * batch_idx / len(train_loader)))
        logger.info(f"loss/d/total:{loss_disc_all}, loss/g/total:{loss_gen_all}, lr:{lr}, grad_norm_d:{grad_norm_d}, grad_norm_g:{grad_norm_g}, loss/g/fm:{loss_fm},loss/g/mel:{loss_mel}, loss/g/dur:{loss_dur}, loss/g/kl:{loss_kl}, loss/g/sup:{loss_sup}, loss/g/unsup:{loss_unsup}, loss/g/emotion:{loss_emotion}")
        scalar_dict = {"loss/g/total": loss_gen_all, "loss/d/total": loss_disc_all, "learning_rate": lr, "grad_norm_d": grad_norm_d, "grad_norm_g": grad_norm_g}
        scalar_dict.update({"loss/g/fm": loss_fm, "loss/g/mel": loss_mel, "loss/g/dur": loss_dur, "loss/g/kl": loss_kl, "loss/g/sup":loss_sup, "loss/g/unsup":loss_unsup,"loss/g/emotion":loss_emotion})
    ...
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, sid, eid, eeg) in enumerate(eval_loader):
        ...
        with autocast(enabled=hps.train.fp16_run):
          ...
          ) = net_g(x, x_lengths, spec, spec_lengths, sid, temp, eeg)
        ...
        if logger is not None:
          logger.info(
            ...
            "loss/g/unsup={:.6f}, loss/g/emotion={:.6f}".format(
              global_step,
