...
from EEG_Encoder_CNN_Transformer_noalign import EEGFeatureExtractor
...
class EEGProjectLinearMean(nn.Module):
    """
    Only Linear projection + temporal mean pooling
    Input : [B, chans, T]
    Output: [B, 256, 1]
    """
    def __init__(self, chans=30, out_dim=256):
        super().__init__()
        self.proj = nn.Linear(chans, out_dim)
...
class SynthesizerTrn(nn.Module):
  ...
  def __init__(self, 
    ...
    **kwargs):
    ...
    self.eegencoder = EEGProjectLinearMean(chans=30, out_dim=256)
