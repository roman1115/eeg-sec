...
def extract_prosody(wav, sr):
    ...
    return {"f0": mean_f0, "energy": energy, "duration": duration}
...
# ==================================================
# 4.5 Build seen-only prototypes
# ==================================================
def build_emotion_prototypes(
    ...
    """
    Build emotion prototypes ONLY from proto_samples (e.g., seen/train set).
    max_per_emotion: 0 means use all; otherwise limit number per emotion for speed.
    """
    ...
    for s in proto_samples:
        emo = s["emotion"]
        if max_per_emotion > 0 and counter[emo] >= max_per_emotion:
            continue

        wav_path = Path(proto_wav_root) / s["wav"]
        emb = ser_embedding(ser_model, wav_path, hps, device)
        tmp[emo].append(emb)
        counter[emo] += 1

    emotion_prototypes = {}
    for emo in sorted(tmp.keys()):
        emotion_prototypes[emo] = torch.stack(tmp[emo]).mean(dim=0)

    # 打印一下每类 prototype 用了多少条（论文里也能用）
    print("\n====== Prototype Build (seen-only) ======")
    for emo in sorted(counter.keys()):
        print(f"{EMO_MAP.get(emo, emo)} : {counter[emo]} samples")
...
def run_single_conversion(
    ...
    if ser_model is not None:
        ...
        # 若某类 prototype 不存在（比如 proto_filelist 没包含某类），跳过
        if emo not in emotion_prototypes:
            return
...
def run_group_inference(args):
    ...
    # ------------ build prototypes from seen-only filelist ------------
    emotion_prototypes = {}
    if ser_model is not None:
        proto_file = args.proto_filelist if args.proto_filelist else args.filelist
        proto_root = args.proto_wav_root if args.proto_wav_root else args.wav_root

        proto_samples = parse_filelist(proto_file)
        emotion_prototypes = build_emotion_prototypes(
            ser_model=ser_model,
            proto_samples=proto_samples,
            proto_wav_root=proto_root,
            hps=hps,
            device=device,
            max_per_emotion=args.proto_max_per_emo,
        )
    ...
    if ser_model is not None and stats["ser"]["total"] > 0:
        ...
        for emo in sorted(stats["ser"]["per_emo"]):
            ...
            print(f"Neutral → {EMO_MAP[emo]}: {acc*100:.2f}% ({correct}/{total})")
...
def main():
    ...
    parser.add_argument("--filelist", required=True, help="Evaluation filelist (e.g., unseen speakers)")
    ...
    # --- NEW: seen-only prototypes ---
    parser.add_argument("--proto-filelist", default=None,
                        help="Filelist used to build emotion prototypes (recommend: seen/train filelist). "
                             "If not set, fallback to --filelist (old behavior).")
    parser.add_argument("--proto-wav-root", default=None,
                        help="wav_root for proto-filelist. If not set, fallback to --wav-root.")
    parser.add_argument("--proto-max-per-emo", type=int, default=0,
                        help="Limit prototype samples per emotion for speed. 0 means use all.")
